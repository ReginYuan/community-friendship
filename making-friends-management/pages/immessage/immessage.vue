<template>
	<view class="p-5">
		<search-bar placeholder="消息内容" @search="submitSearch" />
		<!-- 操作按钮 -->
		<view class="flex py-3" v-permission="'admin.ImMessage/delete,POST'">
		...
		<el-table :data="data" border stripe style="width: 100%" @selection-change="handleSelectionChange" ref="table"
			...
			<el-table-column prop="type" label="类型" width="70">
				<template #default="{ row }">
					<el-tag size="small" style="border:0;">{{ type_text[row.type] }}</el-tag>
				</template>
			</el-table-column>
			<el-table-column prop="body" label="消息内容" width="150" />
			<el-table-column prop="is_push" label="推送" width="60">
				<template #default="{ row }">
					<el-tag size="small" style="border:0;" :type="row.is_push ? 'success' : 'info'">{{ row.is_push ? '是' : "否" }}</el-tag>
				</template>
			</el-table-column>
			<el-table-column prop="state" label="发送状态" width="100">
				<template #default="{ row }">
					<el-tag :type="state_color[row.state]" size="small">{{ state_text[row.state] }}</el-tag> 
				</template>
			</el-table-column>
			...
			<el-table-column prop="create_time" label="发送时间" width="220" />
...
<script>
	export default {
		...
		methods: {
			...
			loadData(keyword = "") {
				this.$loading.show()
				this.$api.getImmessageList(this.current_page, keyword)
					.then(({
						data
					}) => {
						this.data = data.data
